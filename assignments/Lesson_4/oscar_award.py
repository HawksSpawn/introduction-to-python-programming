nominated = {
                1931 : ['Norman Taurog', 'Wesley Ruggles', 'Clarence Brown', 'Lewis Milestone', 'Josef Von Sternberg'], 
...
for year, nominations in nominated.items():
    for director in nominations:
...
for year, winner in winners.items():
    for director in winner:
        if director in count_win_director:
            count_win_director[director] += 1
        else:
            count_win_director[director] = 1

max_wins = max(count_win_director.values())

for director, most_wins in count_win_director.items():
    if most_wins == max_wins:
